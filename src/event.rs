...
#[cfg(unix)] use std::os::unix::{RawFd, AsRawFd};
use std::ptr::null;
...
impl Event
{
	...
	pub fn set(&self)
	{
		...
		#[cfg(unix)] unsafe { let inc: u64 = 1; ::libc::write(self.0, ::std::mem::transmute::<_, *const [u8; 8]>(&inc), 8); }
	}
	pub fn wait(&self)
	{
		#[cfg(windows)] unsafe { ::winapi::um::synchapi::WaitForSingleObject(self.0, ::winapi::um::synchapi::INFINITE); }
		#[cfg(unix)] unsafe { let mut inc: [u8; 8] = [0; 8]; ::libc::read(self.0, inc.as_ptr(), 8); }
	}
}
